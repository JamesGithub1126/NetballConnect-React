...
class AddUmpire extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      ...
      if (!!this.state.isEdit) {
        const teamIds = uniqBy(this.props.umpireState.umpireData.teamId);
        this.formRef.current.setFieldsValue({
  ...
  onChangeOrganisation = affiliateIds => {
    ...
    // Get selected team object list
    const selectTeamList = teamsList.filter(team => {
      return umpireData.teamId.includes(team.id);
    });

    // Selects only teams belonging to the currently selected organizations.
