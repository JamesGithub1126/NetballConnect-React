...
import { Affiliate_CompetitionCreation } from '@pages/affiliateCompetitionCreation.page';
...
test('#WN-435 Registration Competition Creation', async ({ page }) => {
  ...
  const competitionCreation = new Affiliate_CompetitionCreation(page);
  await competitionCreation.competitionCreation(0);
  await competitionCreation.verifyCompetitionIsCreated();
